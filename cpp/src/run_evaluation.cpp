...
void Callback(const sensor_msgs::ImageConstPtr& image_msg, const sensor_msgs::PointCloud2ConstPtr& pc_msg, const sensor_msgs::PointCloud2ConstPtr& result_msg) {
    ...
    MatrixXf gt_nodes = trackdlo_eval.get_ground_truth_nodes(cur_image_orig, cloud_xyz);
    ...
    Y_true = trackdlo_eval.sort_pts(gt_nodes, head_node);
    ...
    // compute error
    double E1 = trackdlo_eval.get_piecewise_error(Y_track, Y_true);
    double E2 = trackdlo_eval.get_piecewise_error(Y_true, Y_track);
...
int main(int argc, char **argv) {
    ...
    std::string bagfile_dir = "/home/jingyixiang/cdcpd2_ws/with_gripper_parallel.bag";

    rosbag::Bag bag(bagfile_dir, rosbag::bagmode::Read);
    ...
    message_filters::Subscriber<sensor_msgs::PointCloud2> result_sub(nh, "/trackdlo_results_pc", 10);

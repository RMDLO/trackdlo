...
```
  ├── trackdlo/   # the TrackDLO class and the ROS node for running it in real-time
  ├── launch/     # ROS launch files that start the Realsense D435 camera and the rviz visualization window
  ├── rviz/       # rviz configurations for tracking result visualization
  ├── config/     # the Realsense D435 camera configurations we used for experiments
  └── utils/      # miscellaneous scripts we used for testing
```
...
## Run TrackDLO with your own setup:
...
**Note that we assume the received RGB image and depth image are ALIGNED and SYNCHRONIZED. That is, depth_image(i, j) should have the depth value of pixel rgb_image(i, j) and the two images should be published with the same ROS timestamp by the camera.**
...
* `/trackdlo/results_img`: an RGB image with tracking results projected onto the received input RGB image
